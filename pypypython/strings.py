...
'''
date = '12. 10. 2020'
den,mesic,rok = date.split(". ",)
print(den+'-'+mesic+'-'+rok)
#//////////////////////////////////////////////
souslovi = 'To je promenna v Pythonu'
souslovi_smol = souslovi.lower()
souslovi_python = souslovi_smol.replace(" ", "_")
#////////////////////////////////////
souslovi_capital = souslovi.title()
souslovi_js2 = souslovi_capital[2:]
souslovi_js1 = souslovi_capital[0].lower() + souslovi_capital[1]
souslovi_capital = souslovi_capital.replace(" ","")
print(souslovi_js1 + souslovi_capital)
print(souslovi_python)
#//////////////////////////////////////////
import random
import string
print("Zadej pocet osob, pro ktere chces mit nahodne hesla: ")
x=0
pocet_osob = input()
pismenaCisla = 3
def randLetter(pismenaCisla):
    letters = string.ascii_letters
    vysledek =''.join(random.choice(letters) for i in range(int(pismenaCisla)))
    return vysledek
def randNumero(pismenaCisla):
    numbers = '1234567890'
    number = ''.join(random.choice(numbers) for i in range(int(pismenaCisla)))
    return number
def randZnak():
    znaky = '*/-+(?:!\#&@'
    znak = ''.join(random.choice(znaky))
    return znak

while x < int(pocet_osob):
    bigLetters = randLetter(pismenaCisla).upper()
    smolletters = randLetter(pismenaCisla).lower()
    print(bigLetters + smolletters + randZnak() + randNumero(pismenaCisla))
    x+=1
